...
网关是长时间运行的服务，改配置、升级代码是常态。直接 kill 掉进程再启动，所有正在进行的请求全部中断——线上完全不可接受。平滑重启要一次解决三件事：**新配置生效、进行中的请求不丢、新旧进程无缝交接**。

整体思路一句话：**新 worker 先上线接客，旧 worker 把手头活干完再下班**——像餐厅换班，新服务员先到位开始接待，旧服务员把已入座的客人服务完才走。

完整流程（SIGHUP 触发，共六步）：

1. **Master 重读配置**：重新解析 config.ini。解析失败就保留旧配置、取消本次重启，服务不受任何影响；
2. **配置热更**：新配置序列化写入共享内存的**非活跃槽**，version++，原子切换 active_slot——快照已换新（worker 实际用到的配置在第 4 步 fork 时继承，这一步让共享内存里的快照先就位）；
3. **发出排空令**：drain_generation++（重启代数 +1），shutting_down 置 1。注意：这**不是信号**，是共享内存里的原子标志——不打扰任何进程，所有 worker 下个周期就能读到；
4. **新 worker 上线**：Master fork 新 worker，它通过 fork 继承新配置（整份进程内存复制给子进程），SO_REUSEPORT 绑同一端口，**立刻开始接新连接**；
5. **旧 worker 排空退出**：旧 worker 的 epoll_wait 每 100ms 超时醒来一次，读到排空令且判断"代数比我新"后，停止 accept（不再接新客），把手头所有请求处理完，主动退出进程；
6. **Master 确认收尾**：Master 用 waitpid 每秒轮询旧 worker 是否退出，全部退出即排空完成，复位 shutting_down，重启结束。若 30 秒还有没退完的（比如某条连接一直不结束），SIGKILL 强杀，并记录 leaked 数量。

三个关键设计，每个对应一个真问题：

1. **双缓冲配置切换**（解决"读配置读到一半"）：新配置先写满非活跃槽，再原子切换 active_slot 指向它。任何进程任何时候读到的都是完整快照——要么旧的要么新的，绝不可能是写了一半的混合体。不靠锁，靠"先写满、再切换"的顺序保证正确性。
2. **generation 计数器**（解决"新 worker 误判排空"）：旧 worker 排空期间 shutting_down 一直是 1，新 worker 启动时也会读到它——光有标志不够，必须能区分"这个排空令是发给谁的"。于是每次重启代数 +1，worker 启动时记录自己的代数，只有"当前代数 > 我的代数"才响应排空。新 worker 代数等于当前值，天然免疫（踩坑 1 讲的就是这个 bug 怎么踩的）。
3. **存量连接排空 + 超时兜底**（解决"连接谁来处理、等多久"）：存量连接的 fd 在 fork 前就属于旧 worker，新 worker 根本碰不到它们——所以"不丢连接"的本质不是"转交连接"，而是"旧 worker 处理完才退出"。Master 等 30 秒是兜底：万一有连接赖着不走，强杀保证重启必然收敛，不会新旧并存拖死系统。

连接不丢的本质就一句话：**停止 accept 只影响新连接，进行中的请求由旧 worker 处理完毕才退出**。实测单轮/两轮/多轮连续热重启 `leaked=0`，没有丢过任何存量连接。
