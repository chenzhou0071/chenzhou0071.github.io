...
服务端有个硬约束"代码里不写死任何数字"，客户端一开始把 `host/port` 写死在了默认值里——风格不一致。补上：`StreamingAssets/network_config.json` 放连接参数，启动时加载，缺失时 fallback 默认值——StreamingAssets 打包后原样保留在安装目录，改配置不用重新打包。客户端和服务器一样，**连接参数来自外部配置**。
